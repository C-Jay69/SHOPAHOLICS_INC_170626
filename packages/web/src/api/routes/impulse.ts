    ...
    try {
      const resp = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: `You are the Shopaholics Anonymous Mate — a witty, sarcastic but genuinely helpful AI shopping coach. Give a short, punchy 1-2 sentence reaction to what's in someone's cart. Be cheeky and British-sarcastic but actually helpful. Suggest saving to wishlist or cooling off if it seems impulsive. Keep it to 1-2 sentences max. No hashtags, no emojis.` },
            { role: "user", content: `Cart ${timeContext}: ${cartSummary}. Total: $${totalAmount.toFixed(2)}. Give me your honest, sarcastic take.` }
          ],
          max_tokens: 100,
          temperature: 0.8,
        }),
      ...
      if (user) {
        await db.insert(schema.impulseCoachStats).values({ userId: user.id, event: "analyzed", savedAmount: 0 });
      }
